...
class BroadcastChatAdminView(TemplateView):
    template_name = 'broadcast_chat_facility.html'

    def get(self, request, *args, **kwargs):
        welcome = RedisMessage('Hello everybody')  # create a welcome message to be sent to everybody
        RedisPublisher(facility='admins', broadcast=True).publish_message(welcome)
        return super(BroadcastChatAdminView, self).get(request, *args, **kwargs)
...
class UserChatView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context.update(users=Admin.objects.all())
        return context
